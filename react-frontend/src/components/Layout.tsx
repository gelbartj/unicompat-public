...
export const FromSearchBanner = (props: any) => (
  (props?.query) ? <div className="fromSearchBanner">Not what you were looking for?{" "}
  <Link to={`/search/${props?.query}`}>See all search results.</Link></div>
  : <></>);
  ...
  return (
    <>
      <Masthead searchVal={searchTerm} dispatch={props.dispatch} />
        <div className={`wrap ${props.blockLink ? "blockLink" : ""}`}>{props.children}</div>
